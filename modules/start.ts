...
export default async function () {
  ...
  if (customInstance) {
    ...
    store.player.hls.api[0] = store.api.proxy[0] = store.api.piped[0] = pi;
    if (useInvidious) {
  ...
  if (HLS && !store.player.legacy) (await import("./hls")).default();
  ...
  if (id) {
    ...
    if (isPWA && shareAction === "watch") {
      ...
      import("../components/WatchVideo.ts").then((mod) => mod.default(dialog));
  ...
  if (params.has("q")) {
    ...
    if (params.has("f")) searchFilters.value = params.get("f") || "";
  ...
  if (params.has("channel") || params.has("playlists"))
    fetchList("/" + location.search.substring(1).split("=").join("/"));
  ...
  (
    ...
    d.addEventListener("click", (e) => {
      ...
      // TODO
      if (!elm.matches("i") && d.open) d.removeAttribute("open");
    });
  });
}
