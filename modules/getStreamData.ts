...
export default async function (
  ...
  const fetchDataFromPiped = (api: string) =>
    fetch(`${api}/streams/${id}`)
      .then((res) => res.json())
      .then((data) => {
        if (state.HLS ? data.hls : data.audioStreams.length) return data;
        else throw new Error(data.message);
      });

  const fetchDataFromInvidious = (api: string) =>
    fetch(`${api}/api/v1/videos/${id}`)
      .then((res) => res.json())
      .then((data) => {
        if (data && "adaptiveFormats" in data) return data;
      ...
      .then((data: Invidious) => ({
        ...
        videoStreams: data.adaptiveFormats
  ...
  const emergency = (e: Error) =>
    !prefetch && fallback ? fetchDataFromPiped(fallback).catch(() => e) : e;

  const useInvidious = (index = 0): Promise<Piped> =>
    status === "N"
      ? Promise.allSettled(
          invidious.filter(Boolean).map(fetchDataFromInvidious)
        ).then((res) => {
      ...
      : fetchDataFromInvidious(invidious[index]).catch((e) => {
          if (index + 1 === invidious.length) return emergency(e);
  ...
  const usePiped = (src: string[], index = 0): Promise<Piped> => {
    ...
    return status === "N"
      ? Promise.allSettled(list.map(fetchDataFromPiped)).then((res) => {
          const ff = res.find((r) => r.status === "fulfilled");
          if (ff?.value) return ff.value as unknown as Piped;
          return useInvidious();
        })
      : fetchDataFromPiped(list[index]).catch(() => {
          if (index + 1 === list.length) return useInvidious();
  ...
  const useHls = () =>
    Promise.allSettled(
      (hls.api.length ? hls.api : piped).filter(Boolean).map(fetchDataFromPiped)
    ).then((res) => {
      ...
      return (
        (ff[0] as PromiseFulfilledResult<Piped>)?.value || {
          message: "No HLS sources are available.",
        }
      );
    });

  const useLocal = async () =>
    await import("./localExtraction").then((mod) => mod.fetchDataFromLocal(id));

  if (location.port === "9999") return useLocal();
  if (state.HLS) return useHls();
  if (status === "I") return useInvidious();

  const primary = (status === "P" ? proxy : piped).filter(Boolean).length
    ? proxy
    : piped;
