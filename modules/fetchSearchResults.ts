...
export const getSearchResults = (query: string, sortBy: string = "") =>
  ...
  const obs = new IntersectionObserver((entries, observer) =>
    entries.forEach(async (e) => {
      if (e.isIntersecting) {
        ...
        if (nextPageToken !== "null") setObserver(callback);
      }
    })
  );
...
function resolvePage(q: string) {
  page = q === previousQuery ? page + 1 : 1;
  return page;
}

export const fetchWithInvidious = (API: string, q: string, sortBy: string) =>
  fetch(`${API}/api/v1/search?q=${q}&sort=${sortBy}&page=${resolvePage(q)}`)
    ...
    .then((items) => {
      ...
      (items as StreamItem[])
        .filter((item) => item.lengthSeconds > 62 && item.viewCount > 1000)
        .forEach((i) => {
          if (results.find((v) => v.videoId === i.videoId) === undefined)
            results.push(i);
        });

      render(searchlist as any, ItemsLoader(results));
      previousQuery = q;
      currentObserver = setObserver(() => fetchWithInvidious(API, q, sortBy));
    });

const loadMoreResults = (API: string, token: string, query: string) =>
  fetch(`${API}/nextpage/search?nextpage=${encodeURIComponent(token)}&${query}`)
    .then((res) => res.json())
    .catch((x) => console.log("e:" + x));

const fetchWithPiped = (API: string, query: string) =>
  fetch(API + "/" + query)
    .then((res) => res.json())
    .then(async (searchResults) => {
      ...
      // filter out shorts
      results = items?.filter((item: StreamItem) => !item.isShort);

      if (searchFilters.value === "music_songs")
        results = results.map((item: StreamItem) => {
