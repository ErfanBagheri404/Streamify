...
import type { StreamItem } from "../types/global";

export default async function fetchList(url: string | undefined, mix = false) {
  if (!url) return notify(i18n("fetchlist_url_null"));
  ...
  const useHyperpipe =
    ...
    (store.actionsMenu.author.endsWith(" - Topic") ||
      store.list.name.startsWith("Artist"));
  ...
  const group = await fetch(api + url)
    ...
    .catch((err) => {
      if (err.message === "Could not get playlistData")
        ...
        errorHandler(mix ? "No Mixes Found" : err.message, () =>
          fetchList(url, mix)
        );
    })
    .finally(() => loadingScreen.close());

  if (!group?.relatedStreams?.length) return;

  if (listContainer.classList.contains("reverse"))
    listContainer.classList.remove("reverse");

  if (musicEnforcer)
    group.relatedStreams = group.relatedStreams.map((item: StreamItem) => {
  ...
  render(listContainer as any, ItemsLoader(listData));
  ...
  if (!mix && group.nextpage)
    setObserver(async () => {
      ...
      data.relatedStreams = data.relatedStreams.filter(
        (item: StreamItem) => !existingItems.includes(item.url.slice(-11))
      );

      if (musicEnforcer)
        data.relatedStreams = data.relatedStreams.map((item: StreamItem) => {
          if (!item.uploaderName.endsWith(" - Topic"))
            item.uploaderName += " - Topic";
          return item;
        });
  ...
  (openInYtBtn.firstElementChild as HTMLParagraphElement).dataset.state =
    " " + group.name;
  ...
  if (!useHyperpipe) {
    ...
    store.list.thumbnail =
      ...
      group.avatarUrl ||
      group.thumbnail ||
      group.relatedStreams[0].thumbnail;
  ...
  (subscribeListBtn.firstElementChild as HTMLParagraphElement).dataset.state =
...
const getPlaylistIdFromArtist = (id: string, index = 0): Promise<string> =>
  fetch(store.api.hyperpipe[index] + id)
    .then((res) => res.json())
    .then((data) => {
      if (!("playlistId" in data)) throw new Error("No Playlist Id found.");
      store.list.id = id.slice(9);
      store.list.type = "channels";
      store.list.thumbnail =
        store.list.thumbnail ||
        "/a-" + data.thumbnails[0]?.url?.split("/a-")[1]?.split("=")[0];
