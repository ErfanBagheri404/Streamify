...
//     src = "songshift",
//     apiIndex = 0
//   ): Promise<(CollectionItem & { src: string }) | undefined> =>
//     await fetch(
//       `${piped[apiIndex]}/search?q=${encodeURIComponent(
//         seed
//       )}&filter=music_songs`
//     )
//       .then((res) => res.json())
//       .then((data) => {
//         if ("items" in data && data.items.length > 0) return data.items[0];
...
//   const promises = songshiftData.map((s) =>
//     getMetadata(`${s.track} ${s.artist}`, s.service)
//   );
//   loadingScreen.showModal();

//   await Promise.all(promises)
//     .then((d) => {
//       d.forEach((v: (CollectionItem & { src: string }) | undefined) => {
//         if (v) {
