import legacyEnTranslations from "../locales/en.json" with { type: "json" };
import legacyFaTranslations from "../locales/fa.json" with { type: "json" };
import { type AppLanguage } from "../lib/app-settings";

type TranslationDictionary = Record<string, unknown>;

const LEGACY_TRANSLATIONS: Record<AppLanguage, TranslationDictionary> = {
  en: legacyEnTranslations as TranslationDictionary,
  fa: legacyFaTranslations as TranslationDictionary,
};
...
export function translate(
  ...
  const localized =
    getNestedValue(LEGACY_TRANSLATIONS[language], key) ||
    getDirectValue(LEGACY_TRANSLATIONS[language], key) ||
    getNestedValue(LEGACY_TRANSLATIONS.en, key) ||
    getDirectValue(LEGACY_TRANSLATIONS.en, key);
