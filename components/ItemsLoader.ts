...
// import type { StreamItem as StreamItemType } from "../types/global";
...
// export default function (itemsArray: string | StreamItemType[]) {
//   const collectionTemplate = (name: string) => html`
//     <a href=${"./list?collection=" + name} class="clxn_item"
//       >${name in reservedCollections
//         ? html`<i class=${reservedCollections[name as "history"][0]}></i>${i18n(
//               reservedCollections[name as "history"][1] as "library_history"
//             )}`
//         : html`<i class="ri-play-list-2-fill"></i>${name}`}</a
//     >
//   `;
//   const template = (item: StreamItemType) =>
...
//           duration:
//             (item.duration || item.lengthSeconds) > 0
//               ? convertSStoHHMMSS(item.duration || item.lengthSeconds)
//               : "LIVE",
//           uploaded: item.uploadedDate || item.publishedText,
//           channelUrl: item.uploaderUrl || item.authorUrl,
//           views:
//             item.viewCountText ||
//             (item.views > 0 ? numFormatter(item.views) + " views" : ""),
//           img: getThumbIdFromLink(
//             item.thumbnail ||
//               "https://i.ytimg.com/vi_webp/" +
//                 item.videoId +
//                 "/mqdefault.webp?host=i.ytimg.com"
//           ),
//         })
//       : ListItem({
//           title: item.name,
