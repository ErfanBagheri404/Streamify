...
//     store.actionsMenu.id
//   )) as unknown as Piped & {
//     videoStreams: Record<"url" | "codec" | "resolution" | "quality", string>[];
//   };
//   const hasAv1 =
//     data.videoStreams.filter((v) => v.codec?.includes("av01")).length === 4
//       ? data.videoStreams.find((v) => v.codec?.includes("av01"))?.url
//       : false;
//   const hasVp9 = data.videoStreams.find((v) => v.codec?.includes("vp9"))?.url;
//   const hasOpus = data.audioStreams.find((a) =>
...
//             <track
//               src=${
//                 (store.api.status === "P" ? "" : store.api.invidious[0]) + v.url
//               }
//               srclang=${v.name}
//             >
//             </track>
...
//                 setState(
//                   "watchMode",
//                   _.target.selectedOptions[0].textContent as string
//                 );
//             },
//             children: html` <option>Video</option>
//               ${media.video.map(
//                 (f) => html`
...
//               )}`,
//             onmount: (_) => {
//               if (savedQ) video.src = proxyHandler(_.value, true);
//             },
//           })
//         : ""}
...
//       <br />
//       <i>
//         Because video streaming consumes a lot of energy, contributing to carbon
//         emissions, please try to watch only what's necessary. When you do
//         stream, select the lowest resolution that meets your needs.</i
//       >
//     </div>
//   `;

//   render(dialog as any, html` ${videoTemplate} ${footerTemplate} `);
