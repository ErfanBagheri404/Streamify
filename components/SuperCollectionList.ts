...
// import type { SuperCollection, Library, APAC } from "../types/global";

// let { defaultSuperCollection } = state;

// document
//   .getElementById("r." + defaultSuperCollection)
//   ?.toggleAttribute("checked");

// superCollectionList.addEventListener("click", superClick);

// addEventListener("dbchange", (e: Event) => {
//   const { db, change } = (e as CustomEvent).detail;
//   if (change) saveDB(db);
//   main();
// });
...
//   if (elm.value !== "for_you") setState("defaultSuperCollection", elm.value);
...
//   render(superCollectionList as any, template);
// }
...
//   return (keys.length ? keys : reservedCollections)
...
// function loadSubList(db: Library, flag: string) {
...
//   const array = [];
//   const pls = (db as any)[type] as {
//     [index: string]: Record<"name" | "uploader" | "thumbnail" | "id", string>;
//   };
...
//     } else if (name.startsWith(special)) continue;
...
//   "channels" in db
//     ? import("../modules/subfeedGenerator").then((mod) =>
//         mod.default(
//           Object.values(db.channels as any)
//             .filter((c: any) => !c.name.startsWith("Artist -"))
//             .map((c: any) => c.id)
//         )
//       )
