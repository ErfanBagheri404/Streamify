...
// import type { Scheme } from "../types/global";

// const style = document.documentElement.style;
// const cssVar = style.setProperty.bind(style);
// const tabColor = <HTMLMetaElement>(
//   document.head.children.namedItem("theme-color")
// );
// const systemDark = matchMedia("(prefers-color-scheme:dark)");

// const translucent = (r: number, g: number, b: number) =>
//   `rgb(${r},${g},${b},${0.5})`;
...
//       .then((e) =>
//         e(generateImageUrl(store.stream.id, "mq"), !store.player.legacy)
//       )
//       .then(colorInjector);
//   else colorInjector((custom || initColor).split(",").map((s) => parseInt(s)));
// }

// if (state.roundness !== "0.4rem") cssVar("--roundness", state.roundness);

// if (state.loadImage) {
//   if (location.pathname !== "/") themer();
