...
// import type { Routes } from "../types/global";
...
//   if (ai.length) ai.replace(ai[0], ai[0].replace("line", "fill"));
...
//     if (aip.length) aip.replace(aip[0], aip[0].replace("fill", "line"));
//   }
...
//     history.pushState({}, "", anchor.id + (sParamInHome ? sParam : otherQuery));

//     const routeName = anchor.lastElementChild?.textContent;
//     const homeTitle = store.stream.title || "Home";

//     document.title = (inHome ? homeTitle : routeName) + " - ytify";
//   }
...
//             mod.default(query.split("=")[1].split(" "))
//           )
...
//   const hasStreamQuery =
//     params.has("s") || params.has("url") || params.has("text");
...
//   if (!store.list.id && location.pathname === "/list") {
//     const param = location.search.substring(1).split("=");

//     if (param[0] === "collection") fetchCollection(param[1]);

//     if (param[0] === "list") fetchList("/" + param.join("/"));
//   }
