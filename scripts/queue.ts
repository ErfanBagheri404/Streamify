...
//   return !(hhmmss.length === 2 && parseInt(hhmmss[0]) < 10);
// }

// function filterLT10() {
//   if (removeQBtn.classList.contains("delete")) removeQBtn.click();

//   const items = queuelist.querySelectorAll(
//     ".streamItem"
//   ) as NodeListOf<HTMLElement>;
//   items.forEach((el) => {
//     const duration = el.dataset.duration as string;
//     if (!isLongerThan10Min(duration)) return;
...
//   for (const item of Array.from(queuelist.children))
...
//       shuffleBtn = el;
//       if (state.shuffle) shuffleBtn.className = "on";
//     }}
//     @click=${shuffle}
//   >
//     <i
...
//     ref=${(el: HTMLButtonElement) => {
//       filterLT10Btn = el;
//       if (state.filterLT10) filterLT10Btn.className = "on";
//     }}
//     @click=${(e: MouseEvent) => {
...
//     if (list.includes(data.id)) return;

//   if (filterLT10Btn.classList.contains("on"))
//     if (isLongerThan10Min(data.duration as string)) return;

//   if (firstChild()?.matches("p")) firstChild()?.remove();

//   if (removeQBtn.classList.contains("on")) removeQBtn.click();

//   if (prepend) list.unshift(data.id);
...
//   if (prepend) queuelist.prepend(fragment);
//   else queuelist.appendChild(fragment);
// };
...
//     dataset: DOMStringMap & CollectionItem;
//   };
...
//   if (state.shuffle && !removeState) shuffle();
// });
