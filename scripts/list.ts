...
//     dataArray.splice(e.newIndex, 0, [oldKey, oldItem]);
...
//   } else if (btn === enqueueBtn) listToQ(listContainer);
//   else if (btn === openInYtBtn)
//     open(
//       ["https://youtube.com", location.origin].includes(store.linkHost)
//         ? "https://youtube.com" + store.list.url
//         : hostResolver(store.list.url)
//     );
//   else if (btn === subscribeListBtn) subscribeList(db);
//   else if (btn === removeFromListBtn) {
//     listContainer
//       .querySelectorAll(".streamItem")
//       .forEach((e) => e.classList.toggle("delete"));
//     removeFromListBtn.classList.toggle("delete");
//   } else if (btn === importListBtn) importList();
//   else if (btn === deleteCollectionBtn) {
//     if (!db) return;

//     if (!confirm(i18n("list_prompt_delete", id))) return;
//     store.addToCollectionOptions.splice(atcIdx, 1);
//     delete db[id];
//     saveDB(db, "delete");
//     history.back();
//   } else if (btn === clearListBtn) {
//     if (!confirm(i18n("list_prompt_clear", id))) return;
//     delete db[id];
//     saveDB(db);
//     history.back();
...
//     delete db[id];
//     saveDB(db, "rename");
//   } else if (btn === shareCollectionBtn) shareCollection(db[id]);
//   else if (btn === radioCollectionBtn)
//     import("../modules/supermix").then((mod) =>
//       mod.default(Object.keys(db[id]))
//     );
...
//     sort("title");
//   } else if (btn === sortByAuthorBtn) sort("author");

//   function sort(field: keyof CollectionItem | "" = "") {
//     let clxnArr = Object.values(db[id]) as CollectionItem[];
//     if (field) {
//       clxnArr = clxnArr.sort((a, b) => {
//         if (a[field as keyof CollectionItem] > b[field as keyof CollectionItem])
//           return 1;
//         if (a[field as keyof CollectionItem] < b[field as keyof CollectionItem])
//           return -1;
//         return 0;
...
//       const ico = (field === "title" ? sortByTitleBtn : sortByAuthorBtn)
...
//       }
//       ico.classList.replace(cls + state[0], cls + state[1]);

//       db[id] = Object.fromEntries(
//         clxnArr.map((v: CollectionItem) => [v.id, v])
//       );
//       saveDB(db);
//     }
