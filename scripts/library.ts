...
// import type { FileEv } from "../types/global";
...
//     }}
//   >
//     <i class="ri-download-cloud-line"></i>&nbsp;${i18n(
...
//       type="file"
//       id="upload_songshift"
//       @change=${async (e: FileEv) =>
//         (await import("../modules/importSongshiftStreams")).default(
//           e.target.files[0]
//         )}
//     />
//   </li>
...
//   for (const collection in newDB)
//     for (const item in newDB[collection])
...
//     JSON.stringify(getDB(), undefined, 2)
//   )}`;
//   link.click();
// }

// function cleanLibrary() {
//   const count = Object.values(getDB()).reduce(
//     (acc: number, collection) =>
//       acc + Object.keys(collection as Record<string, unknown>).length,
//     0
//   );
...
//   if (favButton.checked) addToCollection("favorites", store.stream);
//   else removeFromCollection("favorites", store.stream.id);
...
// if (dbsync)
//   import("../modules/cloudSync").then((mod) => mod.default(dbsync, syncBtn));
// else syncBtn.remove();
